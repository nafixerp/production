...
<div class="card-gold">
    <div class="card-header-gold">
        <h5>◆ New Sales Invoice &nbsp;<small style="font-size:0.75rem;color:var(--text-muted-gold)">{{ $nextSlno }}</small></h5>
    ...
    <form method="POST" action="{{ route('sales.store') }}" id="salesForm">
        ...
        <table class="items-table table-gold w-100" id="itemsTable">
            ...
            <tbody id="itemsBody">
                <tr id="row_0">
                    ...
                    <td><input type="text" name="items[0][unit]" class="form-control" style="width:60px" value="NOS"></td>
            ...
            <div class="col-md-4">
                <div style="background:rgba(212,175,55,0.05);border:1px solid var(--border-gold);border-radius:6px;padding:14px">
                    ...
                    <div class="d-flex justify-content-between mb-1"><span class="form-label">Discount</span>
                        <input type="number" name="discount" id="inp_discount" class="form-control" style="width:110px;text-align:right" value="0" step="0.0001" oninput="calcTotals()">
                    </div>
                    ...
                    <div class="d-flex justify-content-between mb-1"><span class="form-label">Other Charges</span>
                        <input type="number" name="other_charges" id="inp_other" class="form-control" style="width:110px;text-align:right" value="0" step="0.0001" oninput="calcTotals()">
                    </div>
        ...
        <div class="row g-3 mt-2">
            <div class="col-md-3">
                ...
                <input type="number" name="received_amount" class="form-control" value="0" step="0.0001">
            </div>
            <div class="col-md-9">
        ...
        <div class="mt-4">
            ...
            <a href="{{ route('sales.index') }}" class="btn-outline-gold ms-2">Cancel</a>
        </div>
    </form>
</div>
@endsection
...
function calcRow(i) {
    ...
    const amt  = qty * rate;
    const sgstPct = parseFloat(row.querySelector('.item-sgst-pct')?.value) || 0;
    const cgstPct = parseFloat(row.querySelector('.item-cgst-pct')?.value) || 0;
    const igstPct = parseFloat(row.querySelector('.item-igst-pct')?.value) || 0;

    if (row.querySelector('.item-amount')) row.querySelector('.item-amount').value = amt.toFixed(4);
    if (row.querySelector('.item-sgst'))   row.querySelector('.item-sgst').value   = (amt * sgstPct / 100).toFixed(4);
    if (row.querySelector('.item-cgst'))   row.querySelector('.item-cgst').value   = (amt * cgstPct / 100).toFixed(4);
    if (row.querySelector('.item-igst'))   row.querySelector('.item-igst').value   = (amt * igstPct / 100).toFixed(4);

    calcTotals();
}

function calcTotals() {
    let taxable = 0, sgst = 0, cgst = 0, igst = 0;
    document.querySelectorAll('#itemsBody tr').forEach(row => {
        taxable += parseFloat(row.querySelector('.item-amount')?.value) || 0;
    ...
    const other    = parseFloat(document.getElementById('inp_other').value) || 0;
    const net      = taxable - discount + sgst + cgst + igst + other;

    document.getElementById('dispTaxable').textContent = taxable.toFixed(2);
    document.getElementById('dispSgst').textContent    = sgst.toFixed(2);
    document.getElementById('dispCgst').textContent    = cgst.toFixed(2);
    document.getElementById('dispIgst').textContent    = igst.toFixed(2);
    document.getElementById('dispNet').textContent     = net.toFixed(2);

    document.getElementById('inp_taxable').value = taxable.toFixed(4);
    document.getElementById('inp_sgst').value    = sgst.toFixed(4);
    document.getElementById('inp_cgst').value    = cgst.toFixed(4);
    document.getElementById('inp_igst').value    = igst.toFixed(4);
    document.getElementById('inp_net').value     = net.toFixed(4);
}
